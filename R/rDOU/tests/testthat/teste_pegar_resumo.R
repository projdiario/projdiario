...
tmp1 <- pegar_normas_dou(arquivos[[3]], orgao_alvo = "Agricultura") %>%
  criar_tabela_app()
tmp2 <- pegar_normas_dou(arquivos[[5]], orgao_alvo = "Agricultura") %>% criar_tabela_app()
...
test_that('Resumos são como esperados', {
  resumo1 <- '<p>Homologa nos termos do Anexo da presente Portaria, os bens e objetos passíveis de apoio através da transferência de recursos, via contrato de repasse, operacionalizada pela Caixa Econômica Federal (CEF).</p>'
  resumo2 <- '<p>Torna pública a RENÚNCIA EXPRESSA ao benefício da pensão, por ocupar Cargo Público Permanente, com efeitos financeiros a partir de 09/03/2017, conforme o artigo 222, Inciso VI, da Lei nº 8.112/90, formulada por MARTHA CRISTINA</p>'
