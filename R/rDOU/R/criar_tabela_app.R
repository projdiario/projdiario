...
criar_tabela_app <- function(lista_de_atos) {
  ...
  tibble::tibble(
    NUM_ATO = sapply(lista_de_atos, pega_numero), # Ok
    SGL_TIPO = sapply(lista_de_atos, pega_tipo), # Ok
    ...
    COD_TIPO = sapply(lista_de_atos, pega_tipo, 'cod'), # tem que derivar do tipo
    TXT_TEXTO = sapply(lista_de_atos, paste, collapse = "\n") %>% sapply(texto_para_html), # Ok
    DTA_PROMULGACAO = attr(lista_de_atos, 'data_dou'), # Ok
    TXT_EMENTA = sapply(lista_de_atos, pega_resumo), # A principio fora
